...
function SheetForm() {
  ...
  const writeSheetData = async (data) => {
    ...
    data.forEach((row) => {
      const Col2 = row['Column 2'].replaceAll('/','-'); // Replace 'Column C' with the actual name of the column
      console.log(Col2);
      const docRef = db.collection('sheetData').doc(Col2);
      batch.set(docRef, { columnA: row['Column 0'] });
    });
    await batch.commit();
  };
