...
function SheetForm() {
  ...
  db.collection('jobs').doc('test-doc').set(testDocument)
    .then(() => {
      console.log('Test document added to Firestore emulator');
    })
    .catch((error) => {
      console.error('Error adding test document to Firestore emulator:', error);
    });
