...
function SheetForm() {
  ...
  const writeSheetData = async (data) => {
    ...
    let count = 0;
    data.forEach((row) => {
        //TODO: change this so that it is more forgiving allowing open Open open with spaces ect
      if (row['Column 8'] !== "Open") {
        console.log(`Skipping row: ${row['Column 8'] }${row['Column 0']}$`);
